...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  // Fetch quote + renovation + property context via RLS
  const { data: quote } = await supabase
    .from("renovation_quotes")
    .select(
      `
      id, title, description, contractor, total_cost,
      renovations (
        name, classification,
  ...
  const renovation = Array.isArray(quote.renovations)
    ? quote.renovations[0]
    : quote.renovations;
    ...
    const response = await openai.responses.create({
      prompt: {
        ...
        variables: {
          ...
          expensedescription: [quote.title, quote.description]
            .filter(Boolean)
            .join(" — "),
          ...
          expense_context: quote.description ?? "",
          expense_raw_text: "",
        },
      },
      text: {
  ...
  await supabase.from("quote_ai_classifications").delete().eq("quote_id", quoteId);
